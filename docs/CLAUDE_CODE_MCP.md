...
Cette édition utilise Claude Code et ne nécessite pas le chat IA du Studio ni Trigger.dev. Après mise à jour du serveur, reconnecter le client MCP si les nouveaux outils n’apparaissent pas. Le build doit inclure `pnpm prepare:sites` (déjà présent dans `pnpm build`) pour fournir les dépendances du compilateur.

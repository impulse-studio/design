...
export function childLists(node: AnyNode): ChildList[] {
  switch (node.type) {
    ...
    case "component":
    case "template":
      return Object.entries(node.slots ?? {}).map(([key, nodes]) => ({ key, nodes }))
    default:
      return []
  }
}
...
export function walk(node: AnyNode, visit: (node: AnyNode, parent: AnyNode | null, depth: number) => void): void {
...
export type NodeLocation = { node: AnyNode; parent: AnyNode | null; frame: FrameNode; path: AnyNode[] }
...
export function findNode(frames: FrameNode[], id: string): NodeLocation | null {
  for (const frame of frames) {
    ...
    if (search(frame)) {
      return { node: path[path.length - 1]!, parent: path[path.length - 2] ?? null, frame, path }
    }
  }
  return null
}
...
export function nodeLabel(node: AnyNode): string {
  ...
  switch (node.type) {
    case "component": {
      const caption = node.text ?? LABEL_PROPS.map((key) => node.props?.[key]).find((v) => typeof v === "string")
      return caption ? `${node.component} · ${caption}` : node.component
    }
    case "template":
      return node.template
    case "box":
      return node.autoLayout.direction === "row" ? "Auto layout →" : "Auto layout ↓"
    case "text":

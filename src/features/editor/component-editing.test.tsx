...
it("edits a Vue component through the inspector and immediately sends its text, props, paint and dimensions to the renderer", async () => {
  ...
  send.mockClear()
  const content = screen.getByRole("textbox", { name: "Contenu", exact: true })
  await user.clear(content)
  await user.type(content, "Valider")
  await user.tab()
  await user.click(
    screen.getByRole("switch", { name: "disabled", exact: true })
  )
  ...
  const width = screen.getByRole("spinbutton", { name: "W", exact: true })

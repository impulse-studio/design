...
import { createSiteFormSchema } from "@/validators/sites/forms"
...
export function CreateSiteDialog({
  ...
  disabled: boolean
  onCreate: (input: { name: string; kind: SiteKind }) => Promise<void>
}) {
  const [open, setOpen] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const form = useForm({
    defaultValues: { name: "Nouveau site", kind: "react-vite" as SiteKind },
    validationLogic: revalidateLogic({ mode: "blur" }),
    validators: { onDynamic: createSiteFormSchema },
    onSubmit: async ({ value }) => {
      setError(null)
      try {
        await onCreate(createSiteFormSchema.parse(value))
        setOpen(false)
      } catch (reason) {
        setError(
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Un projet indépendant, avec les bibliothèques de votre choix.
          </DialogDescription>
        </DialogHeader>
        <form

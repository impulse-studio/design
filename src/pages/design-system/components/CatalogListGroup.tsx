        ...
        <div className="py-1">
          {entries.map((entry) => (
            <Link
              ...
              className="catalog-entry group/entry flex min-h-10 min-w-0 items-center gap-3 rounded-sm px-3 py-2 [transition:background-color_var(--motion-fast),_color_var(--motion-fast)] motion-reduce:animate-none motion-reduce:[transition:none] [&:hover]:bg-accent [&:hover]:text-foreground [@media(hover:hover)_and_(pointer:fine)]:[&:hover]:bg-accent [@media(hover:hover)_and_(pointer:fine)]:[&:hover]:text-foreground [@media(hover:none)]:[&:hover]:bg-transparent [@media(hover:none)]:[&:hover]:text-inherit"
            >

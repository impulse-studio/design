...
export function SiteCodeView({
  ...
  return (
    <section
      className="site-code-view min-h-0 flex-1 overflow-auto bg-background"
      aria-label={`Code de ${path}`}
    >
      <div className="site-code-toolbar sticky top-0 z-[1] flex items-center justify-between gap-4 border-b border-border bg-background px-4 py-3 text-[12px]">
        <span className="truncate">
          {path}
          {line ? ` · ligne ${line}` : ""}
        </span>
        {canEdit && isEditableFile(path) && code !== undefined && (
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setEditing(!editing)}
          >
            {editing ? "Voir le code" : "Modifier"}
          </Button>
        )}

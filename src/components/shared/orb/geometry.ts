...
export function isLattice(v: OrbVariant): v is LatticeVariant {
  return (LATTICE_VARIANTS as OrbVariant[]).includes(v)
}

export function isRing(v: OrbVariant): v is RingVariant {
  return (RING_VARIANTS as OrbVariant[]).includes(v)
}

export function isHelix(v: OrbVariant): v is HelixVariant {
  return (HELIX_VARIANTS as OrbVariant[]).includes(v)
}

export function isMorph(v: OrbVariant): v is MorphVariant {
...
// lattice is N×N
const PITCH = 6
// centre-to-centre spacing in stage px; the dot size is CSS
const MID = (N - 1) / 2
 ...
 */
const SWIRL = 1.05
// radians of rotation at each end, ~60°
const SPREAD = 1.6
...
export function latticeCells(v: LatticeVariant): Cell[] {
  const cells: Cell[] = []
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
...
export function ringDots(v: RingVariant): RingDot[] {
  const dots: RingDot[] = []
...
const G4_MOVES: RingMove[] = [2, 1, 3, 0, 4, 2, 1, 3, 0, 4].map(
  (ring) => ({
    ring,
...
export function globeDots(v: HelixVariant): GlobeDot[] {
  const dots: GlobeDot[] = []
  let idx = 0
...
function morphShapes(
  v: MorphVariant
): [ShapeFn, ShapeFn, ShapeFn, ShapeFn] {
  switch (v) {
    case "M1":
...
export function morphDots(v: MorphVariant): MorphDot[] {

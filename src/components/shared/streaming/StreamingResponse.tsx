...
export function StreamingResponse({
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      setCopied(false)
      toast.add({ title: "La copie a échoué", type: "error" })
    }
  }, [copyText, onCopy])
        ...
        {shouldShowActions ? (
          ...
          >
            <div className={cn("flex items-center gap-0.5", actionsClassName)}>
              {canCopy ? (
                <ResponseAction
                  label={copied ? "Copié" : "Copier la réponse"}
                  onClick={handleCopy}
                >
                  {copied ? (
              ...
              {onRetry ? (
                <ResponseAction label="Relancer" onClick={onRetry}>
            ...
            {hasSources ? (
              <Collapsible open={currentSourcesOpen}>
                <CollapsibleContent id={sourcesContentId}>
                  <CitationList
                    citations={sources}

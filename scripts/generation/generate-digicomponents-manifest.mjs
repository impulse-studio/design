...
if (!sourceSha)
  throw new Error(
    "--sha is required; run this generator through pnpm sync:digicomponents"
  )

assertExists(bundlePath, "digicomponents build output")
assertExists(
  join(sourceDirectory, "style/main.css"),
  "digicomponents design tokens"
)
...
for (const name of exportNames) {
  const source =
    findComponentSource(join(sourceDirectory, "index.ts"), name) ??
...
const vueSources = [
  ...new Set(
    [...sourceFiles.values(), templatePath].filter((source) =>
      source?.endsWith(".vue")
    )
  ),
]
...
const components = exportNames.map((name) =>
  createComponent(name, sourceFiles.get(name), typeProgram, notes)
)
...
const manifest = {
  ...
  tokens: extractTokens(
    readFileSync(join(sourceDirectory, "style/main.css"), "utf8")
  ),
}

mkdirSync(join(outputRoot, "manifest"), { recursive: true })
writeFileSync(
  join(outputRoot, "manifest/manifest.json"),
  `${JSON.stringify(manifest, null, 2)}\n`
)
writeChangelog(previousManifest, manifest)

console.log(
  `  ${components.length} components, ${templates.length} template, ${Object.values(manifest.tokens).length} token groups`
)
...
function parseArguments(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--sha") {
      const value = args[index + 1]
      if (!value || value.startsWith("--"))
        throw new Error("--sha requires a Git commit SHA")
      parsed.sha = value
      index += 1
      continue
    }
...
function getExportedComponentNames(bundle) {
  const exportBlock = [...bundle.matchAll(/\bexport\s*\{([\s\S]*?)\}\s*;/g)].at(
    -1
  )?.[1]
  if (!exportBlock)
    throw new Error(
      "Could not find the digicomponents export block in dist/index.js"
    )
  return [
    ...new Set(
      exportBlock
        ...
        .filter((name) => name?.startsWith("Digi"))
    ),
  ].sort()
}
...
function findComponentSource(modulePath, exportName, visited = new Set()) {
  ...
  for (const match of source.matchAll(
    ...
    const result = findComponentSource(
...
function findSourceByName(name) {
  for (const path of listFiles(
    sourceDirectory,
    (file) => extname(file) === ".vue" || extname(file) === ".ts"
  )) {
...
function createTypeProgram(files) {
  ...
  host.getSourceFile = (path, ...args) =>
    sourceByVirtualPath.get(resolve(path)) ??
    originalGetSourceFile(path, ...args)
  host.fileExists = (path) =>
    sourceByVirtualPath.has(resolve(path)) || originalFileExists(path)
  host.readFile = (path) => {
...
function createComponent(name, sourcePath, program, notes) {
  ...
  const storyPath = isVueSource
    ? sourcePath.replace(/\.vue$/, ".story.vue")
    : null
  const props = isVueSource ? extractProps(sourcePath, program) : []
  const story =
    storyPath && existsSync(storyPath) ? readFileSync(storyPath, "utf8") : ""
  const previewProps = Object.fromEntries(
    props
      .filter((prop) => prop.required && prop.previewValue !== undefined)
      .map((prop) => [prop.name, prop.previewValue])
  )
  ...
  return {
    ...
    file: sourcePath
      ? relative(library, sourcePath).split(sep).join("/")
      : null,
    description: descriptionFromSource(sourcePath),
    props: props.map(({ previewValue, ...prop }) => prop),
    slots: isVueSource
      ? extractSlots(readFileSync(sourcePath, "utf8"))
      : ["default"],
...
function extractProps(path, program) {
  ...
  let resolvedProps
  try {
    resolvedProps = checker.getPropertiesOfType(
      checker.getTypeFromTypeNode(propsType)
    )
  } catch {
    return []
  }
  ...
  return resolvedProps.map((symbol) => {
    ...
    const required =
      !(symbol.flags & ts.SymbolFlags.Optional) && defaultValue === undefined
    return {
      name: symbol.getName(),
      type: typeName,
      required,
...
function previewValueFor(type) {
  const alternatives = type
    .split("|")
    .map((value) => value.trim())
    .filter((value) => !["undefined", "null", "void"].includes(value))
  const literalString = alternatives.find((value) => /^(['"]).*\1$/.test(value))
  if (literalString) return literalString.slice(1, -1)
  if (
    alternatives.some((value) => ["boolean", "true", "false"].includes(value))
  )
    return false
  if (alternatives.some((value) => value === "number" || /^-?\d/.test(value)))
    return 0
  if (
    alternatives.some(
      (value) => value.includes("[]") || value.startsWith("Array<")
    )
  )
    return []
  ...
  if (
    alternatives.some(
      (value) => value.startsWith("{") || value.startsWith("Record<")
    )
  )
    return {}
  return undefined
...
function typeToString(checker, type, sourceFile) {
  if (type.isUnion()) {
    return type.types
      .filter(
        (part) => !(part.flags & (ts.TypeFlags.Undefined | ts.TypeFlags.Void))
      )
      .map((part) =>
        checker.typeToString(part, sourceFile, ts.TypeFormatFlags.NoTruncation)
      )
      .join(" | ")
  }
...
function extractExamples(story, name) {
  const storyTitle =
    story.match(/<Story\b[^>]*\btitle=["']([^"']+)["']/)?.[1] ?? name
  const variants = [
    ...story.matchAll(
      /<Variant\b[^>]*\btitle=["']([^"']+)["'][^>]*>([\s\S]*?)<\/Variant>/g
    ),
  ]
  ...
  return template
    ? [{ title: storyTitle, exampleCode: cleanExample(template) }]
    : []
}
...
function descriptionFromSource(path) {
  ...
  const source = readFileSync(path, "utf8")
  const comment =
    source.match(/^\s*<!--\s*([\s\S]*?)\s*-->/)?.[1] ??
    source.match(/^\s*\/\*\*\s*([\s\S]*?)\*\//)?.[1]
  return (
    comment
      ?.replace(/^\s*\* ?/gm, "")
      .trim()
...
function categoryFromSource(path) {
  ...
  const categorySegments = relativePath
    .slice(0, -1)
    .filter((part) => part !== "internals")
  if (categorySegments.length === 0)
    return relativePath[0] === "config" ? "Configuration" : "Components"
...
function extractTokens(css) {
  const rootTokens = parseTokenBlock(
    css.match(/:root\s*\{([^}]*)\}/)?.[1] ?? ""
  )
  const darkTokens = parseTokenBlock(
    css.match(/\.dark\s*\{([^}]*)\}/)?.[1] ?? ""
  )
  const select = (tokens, pattern) =>
    Object.fromEntries(
      Object.entries(tokens).filter(([name]) => pattern.test(name))
    )
  return {
    color: select(
      rootTokens,
      /^(?:background|foreground|muted|popover|card|border|input|primary|secondary|success|accent|destructive|off-|blue|purple|warning|white|black|ring)(?:-|$)/
    ),
    darkColor: select(
      darkTokens,
      /^(?:background|foreground|muted|popover|card|border|input|primary|secondary|success|accent|destructive|off-|blue|purple|warning|white|black|ring)(?:-|$)/
    ...
    text: select(
      rootTokens,
      /^(?:base-font-size|font-family|font-scale-ratio|font-size-|font-weight-|letter-spacing-|line-height-)/
    ),
...
function createTemplate(name, path, program) {
  ...
  const source = readFileSync(path, "utf8")
  const props = extractProps(path, program).map(
    ({ previewValue, ...prop }) => prop
  )
  return {
    name,
...
function writeChangelog(previous, current) {
  const path = join(outputRoot, "manifest/CHANGELOG.md")
  const previousByName = new Map(
    (previous.components ?? []).map((component) => [component.name, component])
  )
  const currentByName = new Map(
    current.components.map((component) => [component.name, component])
  )
  const added = [...currentByName.keys()].filter(
    (name) => !previousByName.has(name)
  )
  const removed = [...previousByName.keys()].filter(
  ...
  for (const [name, component] of currentByName) {
    ...
    const oldProps = new Map(
      (previousComponent.props ?? []).map((prop) => [prop.name, prop])
    )
    const newProps = new Map(component.props.map((prop) => [prop.name, prop]))
    const addedProps = [...newProps.keys()].filter(
      (prop) => !oldProps.has(prop)
    )
    const removedProps = [...oldProps.keys()].filter(
      (prop) => !newProps.has(prop)
    )
    const changedProps = [...newProps.keys()].filter((prop) => {
      ...
      return (
        ...
        (oldProp.type !== newProp.type ||
          oldProp.required !== newProp.required ||
  ...
  if (!previous.components) {
    lines.push(
      `- Initial sync: ${current.components.length} exported components and ${current.templates.length} shell template.`
    )
  } else if (
    added.length === 0 &&
    removed.length === 0 &&
    ...
    if (removed.length)
      lines.push(`- Removed components: ${removed.join(", ")}.`)
    for (const change of propChanges) {
      const details = [
        change.addedProps.length ? `added ${change.addedProps.join(", ")}` : "",
        change.removedProps.length
          ? `removed ${change.removedProps.join(", ")}`
          : "",
        change.changedProps.length
          ? `changed ${change.changedProps.join(", ")}`
          : "",
      ].filter(Boolean)
  ...
  const previousPath = join(root, "manifest/CHANGELOG.md")
  const existing = existsSync(previousPath)
    ? readFileSync(previousPath, "utf8").replace(
        /^# Component library changelog\n\n?/,
        ""
      )
    : ""
  const content = existing.includes(`· ${current.orchestrationSha}`)
    ? existing
    : `${section}\n${existing}`

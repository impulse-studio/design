...
import { parseSnapshotArguments } from "./snapshot-options.mjs"
...
const vueRouter = await import(
  pathToFileURL(require.resolve("vue-router")).href
)
const renderer = await import(
  pathToFileURL(require.resolve("vue/server-renderer")).href
)
const digi = await import(
  pathToFileURL(resolve(libraryPath, "dist/index.js")).href
)
...
const manifest = JSON.parse(
  readFileSync(resolve(outputRoot, "manifest/manifest.json"), "utf8")
)
const output = resolve(
  outputRoot,
  "renderer/src/digicomponents.react.generated.json"
)
const reactPackage = resolve(outputRoot, "packages/digicomponents-react")
const reactSource = resolve(reactPackage, "src")
mkdirSync(reactSource, { recursive: true })
const safeName = /^[A-Za-z][A-Za-z0-9]*$/
const contentProps =
  /^(?:text|title|description|label|placeholder|message|caption|tooltip|(?:model)?value|defaultvalue|href|src|url|alt|name|link|route|content|.*(?:Text|Title|Description|Label|Placeholder|Message|Caption|Tooltip|Value|Href|Src|Url|Alt|Link|Route|Content))$/i
...
console.log(
  `  ${Object.keys(components).length} React render maps written to ${output}`
)
...
async function renderVariants(entry, component) {
  ...
  for (const prop of entry.props) {
    if (prop.default !== undefined) defaults[prop.name] = prop.default
      ...
      else if (prop.required && prop.type === "string")
        defaults[prop.name] = "Exemple"
      else if (prop.required && prop.type === "number") defaults[prop.name] = 0
      else if (prop.required && /^(?:boolean|false \| true)$/.test(prop.type))
        defaults[prop.name] = false
    }
  }
  if (
    entry.name === "DigiTabsContainer" &&
    Array.isArray(defaults.tabs) &&
    !defaults.tabs.length
  )
  ...
  for (const selected of combinations) {
    ...
    const target = vue.h(
      ...
      {
        ...props,
        ...(contextFor(entry.name)
          ? { "data-digit-snapshot-target": "true" }
          : {}),
      },
      slots
    )
...
function contextualComponent(name, props, target) {
  ...
  if (name.startsWith("DigiAccordion")) {
    ...
    const item =
      name === "DigiAccordionItem" || name === "DigiAccordionCardItem"
        ? target
  ...
  if (
    /^Digi(?:DashboardCard|GroupClickableCard|GroupDraggableCard|RowClickableCard|RowDraggableCard|SortableHandle)$/.test(
      name
    )
  ) {
  ...
  if (
    /^(?:DigiDateTimePickerFormField|DigiNudeFormField|DigiOptionalDateTimePickerFormField|DigiOptionalSearchSelectFormField|DigiRadioFormField|DigiSearchField)$/.test(
      name
    )
  )
    return {
    ...
    return {
      ...
      node: withDefaultSlot(
        "DigiRadioGroup",
        {
          ...
          modelValue: props.value ?? "digit-preview-item",
        },
        target
      ),
    }
...
function writeReactComponentExports(entries) {
  ...
  const exports = [
    ...
    ...names.map(
      (entry) => `export { ${entry.name} } from "./components/${entry.name}"`
    ),
  ]
  for (const entry of names) {
    const props = entry.props
      ...
      .map(
        (prop) =>
          `  ${JSON.stringify(prop.name)}${prop.required ? "" : "?"}: ${reactType(prop.type)}`
      )
      .join("\n")
    const slotType =
      entry.slots.map((slot) => JSON.stringify(slot)).join(" | ") || "never"
    ...
    exports.push(
      `export type { ${entry.name}Props } from "./components/${entry.name}"`
    )
  }
...
function reactType(type) {
  ...
  if (
    /^(?:string|number|boolean|bigint|symbol|unknown|null|undefined|void|never)(?:\s*\|\s*(?:string|number|boolean|bigint|symbol|unknown|null|undefined|void|never|['"].*?['"]))*$/.test(
      clean
    )
  )
    return clean
  if (
    /^(?:['"].*?['"]|true|false|-?\d+(?:\.\d+)?)(?:\s*\|\s*(?:['"].*?['"]|true|false|-?\d+(?:\.\d+)?))*$/.test(
      clean
    )
  )
    return clean
